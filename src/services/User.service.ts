...
export class UserService {
  ...
  private permissionUserRepository =
    AppDataSource.getRepository(PermissionUser);
  ...
  async createUser(
    ...
    try {
      ...
      if (!passwordRegex.test(password)) {
        throw new Error(
          "The password must be between 8 and 16 characters long, containing at least one saved letter, one lowercase letter, one number and one special character."
        );
      }
      if (existingEmail) {
  ...
  async updatedUser(
    ...
    if (password) {
      ...
      if (!passwordRegex.test(password)) {
        throw new Error(
          "The password must be between 8 and 16 characters long, containing at least one uppercase letter, one lowercase letter, one number, and one special character."
        );
      }
